  ...
  <div className="p-4">
    <div className="flex justify-between items-center mb-4">
      ...
      <div className="flex gap-10 lg:gap-4">
        <Link href={'/cart'} className="flex justify-end text-[#2563eb]">
          View Cart
        </Link>
...
const NavigationLinks = ({
  ...
  <ul
    className={`flex ${
      isMobile ? 'flex-col items-center space-y-6' : 'gap-10 items-center'
    } text-white`}
  >
      ...
      <li key={link.href}>
        <Link
          ...
          className={`!text-white ${
            isMobile ? 'text-xl font-semibold' : 'text-sm'
          } hover:underline`}
          onClick={onLinkClick}
        >
          {link.label}
...
const Header = () => {
  ...
  const showProductButtons = router.pathname.startsWith('/products/')

  return (
    <div className="relative" id="header">
      <header
        className={`header w-full z-50 transition-all duration-300 ${
          isSticky ? 'fixed top-0 left-0 right-0 py-3 bg-[#23284a] shadow-lg' : 'py-3 bg-[#23284a]'
        }`}
      >
        <div className="header__wrapper flex items-center justify-between px-8">
          ...
          <div className="hidden lg:flex gap-4 items-center">
            ...
            {showProductButtons && <ProductActionButtons />}
          </div>
          {/* Mobile Menu Button */}
          <button
            onClick={toggleMenu}
        ...
        <div className="relative h-full">
          ...
          <div className="flex flex-col items-center justify-center h-full space-y-8">
            <nav className="w-full">
              <NavigationLinks isMobile onLinkClick={toggleMenu} />
            </nav>
